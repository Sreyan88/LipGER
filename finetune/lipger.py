...
from lipger.lipger import GPT, Block, Config, adapter_filter, mark_only_adapter_as_trainable
from lipger.speed_monitor import SpeedMonitorFabric as SpeedMonitor
from lipger.speed_monitor import estimate_flops, measure_flops
...
micro_batch_size = 1  # was 6 with 500
...
def setup(
    ...
    checkpoint_dir: Path = Path("/your/path/to/your/checkpoint/"),
...
def train(
    ...
    for iter_num in range(max_iters // gradient_accumulation_iters):
        ...
        for iter in range(gradient_accumulation_iters):
            ...
            input_ids, targets, emb_diff, audio_features, clean_audio_features, visual_features = get_batch_para(
                fabric, model, train_data, longest_seq_length, longest_seq_ix if real_iter == 0 else None, lipreading_preprocessing_func = lipreading_preprocessing_func
            )
            micro_batch.append((input_ids, targets, emb_diff, audio_features, clean_audio_features, visual_features))
        ...
        for iter in range(gradient_accumulation_iters):
            ...
            input_ids, targets, emb_diff, audio_features, clean_audio_features, visual_feature = micro_batch[iter]
            ...
            with fabric.no_backward_sync(model, enabled=(iter + 1) % gradient_accumulation_iters != 0):
                logits, emb_diff_mids = model(input_ids, emb_diff=emb_diff, lip_query=visual_feature, max_seq_length=max_seq_length, lm_head_chunk_size=128)
            ...
            if real_iter % log_interval == 0:
                fabric.print(f"iter {real_iter + 1} step2: loss {loss.item():.4f}, loss_ce {loss_ce.item():.4f}, time: {dt:.2f}s")
...
def validate(
    ...
    for k in range(eval_iters):
        input_ids, targets, emb_diff, visual_features = get_batch(fabric, model, val_data, longest_seq_length, lipreading_preprocessing_func=lipreading_preprocessing_func)
        # logits = model(input_ids)
        logits, emb_diff_mids = model(input_ids, emb_diff=emb_diff, lip_query=visual_features)
...
def get_batch(
    fabric: L.Fabric, model, data: List[Dict], longest_seq_length: int, longest_seq_ix: Optional[int] = None, lipreading_preprocessing_func = None
    ...
    mouth_features = [torch.from_numpy(load_mouthroi(data[i]['mouthroi'])) for i in ix]
    mouth_features, _ = pad_mouth([lipreading_preprocessing_func(item) for item in mouth_features])
    mouth_features = [torch.FloatTensor(item).unsqueeze(0) for item in mouth_features]
    ...
    mroi = torch.stack([x for x in mouth_features], dim=0) #stack
    
    if fabric.device.type == "cuda" and x.device.type == "cpu":
        x, y, ef, mroi = fabric.to_device((x.pin_memory(), y.pin_memory(), ef.pin_memory(), mroi.pin_memory()))
    else:
        x, y, ef, mroi = fabric.to_device((x, y, ef, mroi))
    return x, y, ef, mroi
...
def get_batch_para(
    ...
    labels = [data[i]["labels"][:max_input_length].type(torch.int64) for i in ix]
    emb_diff = [data[i]["emb_diff"].type(model.dtype) for i in ix]
    # audio_features = [torch.from_numpy(np.load(data[i]['noisy_speech'],allow_pickle=True)).type(model.dtype) for i in ix]
    # clean_audio_features = [torch.from_numpy(np.load(data[i]['clean_speech'],allow_pickle=True)).type(model.dtype) for i in ix]
    audio_features = [0 for i in ix]
    clean_audio_features = [0 for i in ix]
    mouth_features = [torch.from_numpy(load_mouthroi(data[i]['mouthroi'])) for i in ix]
    mouth_features, _ = pad_mouth([lipreading_preprocessing_func(item) for item in mouth_features])
    mouth_features = [torch.FloatTensor(item).unsqueeze(0) for item in mouth_features]
    ...
    y = torch.stack([pad_right(x, pad_id=-1) for x in labels])
    ef = torch.stack([x for x in emb_diff], dim=0)
    # af = pad_sequence([x for x in audio_features], batch_first=True) #pad sequence
    # c_af = pad_sequence([x for x in clean_audio_features], batch_first=True) #pad sequence
    af = torch.tensor(audio_features) #pad sequence
    c_af = torch.tensor(clean_audio_features) #pad sequence
    mroi = torch.stack([x for x in mouth_features], dim=0) #stack

    if fabric.device.type == "cuda" and x.device.type == "cpu":
        x, y, ef, af, c_af, mroi = fabric.to_device((x.pin_memory(), y.pin_memory(), ef.pin_memory(), af.pin_memory(), c_af.pin_memory(), mroi.pin_memory()))
    else:
        x, y, ef, af, c_af, mroi = fabric.to_device((x, y, ef, af, c_af, mroi))
    return x, y, ef, af, c_af, mroi
